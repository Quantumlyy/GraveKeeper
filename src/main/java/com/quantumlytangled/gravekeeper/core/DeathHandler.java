...
public final class DeathHandler {
  ...
  public void onPlayerDeath(@Nonnull final LivingDeathEvent event) {
    ...
    if ( !GraveKeeperConfig.IGNORE_KEEP_INVENTORY
      ...
      GraveKeeper.logger.debug(String.format("Keep inventory is enabled, ignoring death of player %s",
          player ));
      return;
    }
    ...
    try {
      ...
      GraveKeeper.logger.info(String.format("Archived DeathChest content for %s, restore it with /gkrestore %s",
          playerName,
          identifier ));
    ...
    // place the grave
    worldPositionGrave.getWorld().setBlockState(worldPositionGrave.blockPos, GraveKeeper.blockDeathChest.getDefaultState());
    ...
    if (!(tileEntity instanceof TileDeathChest)) {
      GraveKeeper.logger.error(String.format("Missing tile entity %s, unable to save player %s inventory in world %s at %s",
    ...
    // log to console
    GraveKeeper.logger.info(String.format("Generated DeathChest for %s (%s) in DIM%d at (%d %d %d).",
