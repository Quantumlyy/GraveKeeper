...
public class InventoryHandler {
  ...
  private static final LinkedHashMap<InventoryType, ICompatInventory> compatInventories = new LinkedHashMap<>(
      10);
      ...
      for (final InventorySlot inventorySlot : inventorySlots) {
        if (inventorySlot.isSoulbound
          ...
          GraveKeeper.logger.warn(
              String.format("Failed to move soulbinded item to main inventory (is it full?): %s",
  ...
  private static void collectOnDeath(@Nonnull final EntityPlayerMP player,
    ...
    for (int index = 0; index < itemStacks.size(); index++) {
      ...
      final boolean isCharmed = CharmHandler
          .isCharmed(charmMode, player.inventory.currentItem, compatInventory.getType(), index,
              itemStack);
      if (isCharmed) {
        if (GraveKeeperConfig.DEBUG_LOGS) {
      ...
      final InventorySlot inventorySlot = new InventorySlot(itemStack, index,
          compatInventory.getType(), isCharmed, isSoulbound);
  ...
  @Nonnull
  public static List<ItemStack> restoreOrOverflow(@Nonnull final EntityPlayerMP player,
      @Nonnull final List<InventorySlot> inventorySlots,
  ...
  private static void restoreOrOverflow(@Nonnull final EntityPlayerMP player,
    ...
    final ICompatInventory compatInventory = compatInventories
        .getOrDefault(inventorySlot.type, CompatMain.getInstance());

    final ItemStack itemStackLeft = compatInventory
        .setItemReturnOverflow(player, inventorySlot.slot, inventorySlot.itemStack);
