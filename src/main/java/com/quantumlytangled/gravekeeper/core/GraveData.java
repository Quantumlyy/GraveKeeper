...
public class GraveData {
  ...
  public GraveData(@Nonnull final List<InventorySlot> inventorySlots,
      @Nonnull final WorldPosition worldPositionPlayer,
      @Nonnull final WorldPosition worldPositionGrave) {
  ...
  public GraveData(@Nonnull final NBTTagCompound tagContainer) {
    ...
    if (!modid.equals(GraveKeeper.MODID)) {
      throw new RuntimeException(
          String.format("Invalid InventorySlots format: unknown modid %s", modid));
    }
    final int version = tagContainer.getInteger("format_version");
    if (version > VERSION) {
      throw new RuntimeException(
          String.format("Invalid InventorySlots format: unknown version %d", version));
    }
