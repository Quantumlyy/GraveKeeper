...
public class GravePosition {
  ...
  @Nonnull
  public static WorldPosition get(@Nonnull final EntityPlayer entityPlayer,
      @Nonnull final BlockPos blockPosInitial) {
    ...
    if (worldPositionResult.blockPos.getY()
        <= GraveKeeperConfig.USE_BED_OR_SPAWN_LOCATION_BELOW_Y) {
    ...
    final boolean isFlying = isFreeSpot(worldPositionResult.getWorld(),
        worldPositionResult.blockPos, false, false);
    ...
    GraveKeeper.logger
        .info(String.format("Player has no safe spot for its grave, releasing direction: %s %s",
    ...
    if (isFlying) {
      if (GraveKeeperConfig.DEBUG_LOGS) {
        GraveKeeper.logger.info(String.format("Assuming void area, defaulting to (%d %d %d)",
            worldPositionResult.blockPos.getX(), worldPositionResult.blockPos.getY(),
            worldPositionResult.blockPos.getZ()));
      }
      return worldPositionResult;
    }
    ...
    final IBlockState blockState = worldPositionResult.getWorld()
        .getBlockState(worldPositionResult.blockPos);
    final TileEntity tileEntity = worldPositionResult.getWorld()
        .getTileEntity(worldPositionResult.blockPos);
    GraveKeeper.logger
        .warn(String.format("Can't find free slot for grave, deleting %s with tile entity %s at %s",
  ...
  private static void updateWithBedOrSpawn(@Nonnull final EntityPlayer entityPlayer,
      @Nonnull final WorldPosition worldPositionResult) {
    ...
    if (blockPosBed != null) {// (bed found in current world)
      ...
      // try spawn dimension
      final World world = FMLCommonHandler.instance().getMinecraftServerInstance()
          .getWorld(GraveKeeperConfig.SPAWN_DIMENSION_ID);
      if (world != null) {
        ...
        GraveKeeper.logger.warn(String.format(
            "Failed to load spawn dimension with id %s, check console for details and review your configuration accordingly.",
  ...
  private static boolean updateWithNearbyFreeSpot(
      @Nonnull final WorldPosition worldPositionResult) {
    if (GraveKeeperConfig.DEBUG_LOGS) {
    ...
    // compute search area
    final int xMin =
        worldPositionResult.blockPos.getX() - GraveKeeperConfig.SEARCH_RADIUS_HORIZONTAL_M;
    final int xMax =
        worldPositionResult.blockPos.getX() + GraveKeeperConfig.SEARCH_RADIUS_HORIZONTAL_M;
    final int yMin = Math
        .max(worldPositionResult.blockPos.getY() - GraveKeeperConfig.SEARCH_RADIUS_BELOW_M,
            GraveKeeperConfig.SEARCH_MIN_ALTITUDE);
    final int yMax = Math
        .min(worldPositionResult.blockPos.getY() + GraveKeeperConfig.SEARCH_RADIUS_ABOVE_M,
            worldPositionResult.getWorld().getHeight() - 1);
    final int zMin =
        worldPositionResult.blockPos.getZ() - GraveKeeperConfig.SEARCH_RADIUS_HORIZONTAL_M;
    final int zMax =
        worldPositionResult.blockPos.getZ() + GraveKeeperConfig.SEARCH_RADIUS_HORIZONTAL_M;
      ...
      for (int z = zMin; z <= zMax; z++) {
        ...
        for (int y = yMin; y <= yMax; y++) {
          ...
          if (isFreeSpot(worldPositionResult.getWorld(), mutableBlockPos, true, false)) {
            ...
            final RayTraceResult rayTraceResult = worldPositionResult.getWorld()
                .rayTraceBlocks(vStarting, vTarget);
            final boolean isHidden =
                rayTraceResult == null || !rayTraceResult.getBlockPos().equals(mutableBlockPos);
            if (isHidden) {
  ...
  // note: only updates worldPositionResult when returning true
  private static boolean updateVertically(@Nonnull final WorldPosition worldPositionResult,
