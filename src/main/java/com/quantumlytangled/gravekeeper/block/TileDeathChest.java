...
public class TileDeathChest extends TileEntity {
  ...
  public void readFromNBT(@Nonnull NBTTagCompound tagCompound) {
    ...
    final NBTTagList nbtInventorySlots = tagCompound
        .getTagList("InventorySlots", Constants.NBT.TAG_COMPOUND);
  ...
  private void doInspection(@Nonnull final EntityPlayer player, final boolean isCreative,
      final boolean isOwner, final long timeRemaining) {
    final ITextComponent textOwner = new TextComponentString(
        ownerName == null ? "-null-" : ownerName);
    textOwner.getStyle()
        .setHoverEvent(new HoverEvent(Action.SHOW_TEXT,
            new TextComponentString(ownerUUID == null ? "-null-" : ownerUUID.toString())))
        .setColor(TextFormatting.AQUA);

    final ITextComponent textDuration = new TextComponentString(
    ...
    final ITextComponent textSize = new TextComponentString(
        String.format("%d", inventorySlots.size()));
    textSize.getStyle()
        .setColor(TextFormatting.AQUA);
  ...
  protected void doDropContent() {
    for (InventorySlot inventory : inventorySlots) {
      final EntityItem entityItem = new EntityItem(world, pos.getX() + 0.5D, pos.getY() + 0.5D,
          pos.getZ() + 0.5D, inventory.itemStack);
  ...
  private void doReturnToOwner(@Nonnull final EntityPlayerMP player) {
    final List<ItemStack> overflow = InventoryHandler
        .restoreOrOverflow(player, inventorySlots, false);
    ...
    for (final ItemStack itemStack : overflow) {
      ...
      world.spawnEntity(
          new EntityItem(world, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D,
              itemStack));
    }
