...
public class BlockDeathChest extends Block {
  ...
  @Override
  public boolean onBlockActivated(@Nonnull final World world, @Nonnull final BlockPos blockPos,
  ...
  @Override
  public void onBlockExploded(@Nonnull final World world, @Nonnull final BlockPos blockPos,
      @Nonnull final Explosion explosion) {
    // do NOT call super, do NOT break the block

    final EntityPlayer player = world
        .getClosestPlayer(blockPos.getX() + 0.5D, blockPos.getY() + 0.5D, blockPos.getZ() + 0.5D, 6,
            false);

    GraveKeeper.logger.warn(
        String.format("Death chest exploded at %s (%d %d %d) due to %s in proximity of player %s",
            WorldPosition.format(world), blockPos.getX(), blockPos.getY(), blockPos.getZ(),
            explosion, player));
  }

  @Override
  public void breakBlock(@Nonnull final World world, @Nonnull final BlockPos blockPos,
      @Nonnull final IBlockState blockState) {
    if (world.isRemote) {
    ...
    final EntityPlayer player = world
        .getClosestPlayer(blockPos.getX() + 0.5D, blockPos.getY() + 0.5D, blockPos.getZ() + 0.5D, 6,
            false);
    GraveKeeper.logger
        .warn(String.format("Death chest broken at %s (%d %d %d) in proximity of player %s",
            WorldPosition.format(world), blockPos.getX(), blockPos.getY(), blockPos.getZ(),
            player));
  ...
  @Nonnull
  @Override
  public TileEntity createTileEntity(@Nonnull final World world,
      @Nonnull final IBlockState blockState) {
  ...
  @Nonnull
  @Override
  public AxisAlignedBB getBoundingBox(@Nonnull final IBlockState blockState,
  ...
  @Nonnull
  @Override
  public BlockFaceShape getBlockFaceShape(@Nonnull final IBlockAccess blockAccess,
      @Nonnull final IBlockState blockState, @Nonnull final BlockPos blockPos,
      @Nonnull final EnumFacing enumFacing) {
