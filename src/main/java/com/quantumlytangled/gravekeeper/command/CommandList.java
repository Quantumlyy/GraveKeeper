...
public class CommandList extends CommandBase {
  ...
  public void execute(@Nonnull final MinecraftServer server,
    ...
    EntityPlayerMP entityPlayer =
        commandSender instanceof EntityPlayerMP ? (EntityPlayerMP) commandSender : null;
      ...
      if (argument.length() == 36) {// 36 chars of UUID
        if (entityPlayer == null) {
          commandSender.sendMessage(getPrefix().appendSibling(
              new TextComponentString("Unable to list graves by UUID from console, sorry.")
        ...
        if (isMissing) {
          commandSender.sendMessage(getPrefix().appendSibling(
              new TextComponentString(String.format(
                  "Unable to find player with name %s: player is offline or name is incorrect. Try using their UUID instead.",
                  argument)).setStyle(new Style().setColor(TextFormatting.RED))));
          return;
        }
    ...
    final File[] rawFiles = fileDirectory
        .listFiles((dir, name) -> name.startsWith(identifier) && name.endsWith(".dat"));
    ...
    final ITextComponent textSender = new TextComponentTranslation(
        "gravekeeper.command.list_header", rawFiles.length, argument);
    textSender.getStyle().setColor(TextFormatting.GREEN);
    commandSender.sendMessage(textSender);

    // check permissions
    final List<ICommand> listCommands = server.getCommandManager()
        .getPossibleCommands(commandSender);
    ...
    for (final File file : sortedFiles) {
      ...
      final String presentedTime =
          elapsedTime.length() > timestampSeconds.length() ? timestampSeconds
              : elapsedTime + " ago";
      ...
      if (canRestore) {
        textTime.getStyle()
            ...
            .setHoverEvent(new HoverEvent(
                HoverEvent.Action.SHOW_TEXT, new TextComponentString(
      ...
      if (graveData == null) {
        ...
        textPosition.getStyle()
            .setHoverEvent(new HoverEvent(
                ...
                new TextComponentString("Check console logs for further details")))
        ...
        if (graveData.worldPositionGrave.isSameWorld(entityPlayer.world)) {
          ...
          if (canTeleportPos) {
            textPosition.getStyle()
                .setClickEvent(new ClickEvent(
                    ClickEvent.Action.SUGGEST_COMMAND, String
                    .format("/tppos %d %d %d", blockPos.getX(), blockPos.getY(), blockPos.getZ())))
                .setHoverEvent(new HoverEvent(
                    HoverEvent.Action.SHOW_TEXT,
                    new TextComponentString("Click to suggest tp command")))
            ...
            textPosition.getStyle()
                .setClickEvent(new ClickEvent(
                    ClickEvent.Action.SUGGEST_COMMAND, String
                    .format("/tp %d %d %d", blockPos.getX(), blockPos.getY(), blockPos.getZ())))
                .setHoverEvent(new HoverEvent(
                    HoverEvent.Action.SHOW_TEXT,
                    new TextComponentString("Click to suggest tp command")))
                .setColor(TextFormatting.AQUA);
          ...
          textPosition.getStyle()
              .setHoverEvent(new HoverEvent(
                  HoverEvent.Action.SHOW_TEXT,
                  new TextComponentString("Enter that dimension before you can teleport")))
