...
public class TileDeathChest extends TileEntity {
    ...
    public void processInteraction(EntityPlayer player, World world, BlockPos pos) {
        ...
        else if ((ChestedGravestonesConfig.getExpiredStatus(creationDate) || ChestedGravestonesConfig.INSTANT_COLLECTION) && !ownerUUID.equals(player.getUniqueID())) processDropItemReturn(player, world, pos);
    }
        ...
        for (InventoryDeathSlot slot : this.contents.inventory) {
            switch(slot.type) {
                ...
                case OFFHAND:
                    ...
                    break;
                case BAUBLES:
                    if (ChestedGravestonesConfig.isBaublesLoaded == null) break;
                    if (ChestedGravestonesConfig.isBaublesLoaded.isSlotEmpty(slot.slot, player)) ChestedGravestonesConfig.isBaublesLoaded.setItem(slot.slot, slot.content, player);
                    else overflow.add(slot.content);
                    break;
                case GCC:
                    if (ChestedGravestonesConfig.isGalacticCraftCoreLoaded == null) break;
                    if (ChestedGravestonesConfig.isGalacticCraftCoreLoaded.isSlotEmpty(slot.slot, player)) ChestedGravestonesConfig.isGalacticCraftCoreLoaded.setItem(slot.slot, slot.content, player);
                    else overflow.add(slot.content);
                    break;
            }
        }
