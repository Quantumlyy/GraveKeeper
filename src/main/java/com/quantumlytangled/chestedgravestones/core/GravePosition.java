...
public class GravePosition {
  ...
  public static WorldPosition get(@Nonnull final EntityPlayer entityPlayer, @Nonnull final BlockPos blockPosInitial) {
    ...
    if (worldPositionResult.blockPos.getY() <= ChestedGravestonesConfig.USE_BED_OR_SPAWN_LOCATION_BELOW_Y) {
    ...
    } else if (worldPositionResult.blockPos.getY() < ChestedGravestonesConfig.SEARCH_MIN_ALTITUDE) {
      worldPositionResult.blockPos = new BlockPos(
          worldPositionResult.blockPos.getX(),
          ChestedGravestonesConfig.SEARCH_MIN_ALTITUDE,
          worldPositionResult.blockPos.getZ() );
    }
    ...
    if (isFlying) {
      if (ChestedGravestonesConfig.DEBUG_LOGS) {
  ...
  private static void updateWithBedOrSpawn(@Nonnull final EntityPlayer entityPlayer, @Nonnull final WorldPosition worldPositionResult) {
    ...
    if (blockPosBed != null) {// (bed found in current world)
      ...
      // try spawn dimension
      final World world = FMLCommonHandler.instance().getMinecraftServerInstance().getWorld(ChestedGravestonesConfig.SPAWN_DIMENSION_ID);
      if (world != null) {
        // try bed location in spawn dimension
        blockPosBed = entityPlayer.getBedLocation(ChestedGravestonesConfig.SPAWN_DIMENSION_ID);
        ...
        Registration.logger.warn(String.format("Failed to load spawn dimension with id %s, check console for details and review your configuration accordingly.",
            ChestedGravestonesConfig.SPAWN_DIMENSION_ID ));
      }
    }
  }
  ...
  private static boolean updateWithNearbyFreeSpot(@Nonnull final WorldPosition worldPositionResult) {
    ...
    // compute search area
    final int xMin = worldPositionResult.blockPos.getX() - ChestedGravestonesConfig.SEARCH_RADIUS_HORIZONTAL_M;
    final int xMax = worldPositionResult.blockPos.getX() + ChestedGravestonesConfig.SEARCH_RADIUS_HORIZONTAL_M;
    final int yMin = Math.max(worldPositionResult.blockPos.getY() - ChestedGravestonesConfig.SEARCH_RADIUS_BELOW_M,
                              ChestedGravestonesConfig.SEARCH_MIN_ALTITUDE );
    final int yMax = Math.min(worldPositionResult.blockPos.getY() + ChestedGravestonesConfig.SEARCH_RADIUS_ABOVE_M,
                              worldPositionResult.world.getHeight() - 1 );
    final int zMin = worldPositionResult.blockPos.getZ() - ChestedGravestonesConfig.SEARCH_RADIUS_HORIZONTAL_M;
    final int zMax = worldPositionResult.blockPos.getZ() + ChestedGravestonesConfig.SEARCH_RADIUS_HORIZONTAL_M;
  ...
  private static boolean updateVertically(@Nonnull final WorldPosition worldPositionResult, final boolean isFlying) {
    ...
    while ( y >= ChestedGravestonesConfig.SEARCH_MIN_ALTITUDE
  ...
  private static boolean isFreeSpot(@Nonnull final World world, @Nonnull final BlockPos blockPos,
    ...
    if ( blockPos.getY() < ChestedGravestonesConfig.SEARCH_MIN_ALTITUDE
    ...
    if ( isAirOrReplaceable
      && ChestedGravestonesConfig.DEBUG_LOGS ) {
